...
RUN mkdir /syncovery && wget -O "$SETUP_TEMP" 'https://www.syncovery.com/release/SyncoveryCL-x86_64-10.16.11-Web.tar.gz' && tar -xvf "$SETUP_TEMP" --directory /syncovery && rm -f "$SETUP_TEMP" && chmod +x /syncovery/SyncoveryCL && chmod +x /entrypoint.sh
